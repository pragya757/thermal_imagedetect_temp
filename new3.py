...
def detect_full_face_temperature(thermal_frame):
...
def capture_image():
    ...
    detected_temperature = detect_full_face_temperature(thermal_frame.copy())

    if detected_temperature is not None:
        matched_disease = match_disease(detected_temperature)

        # Display the comparison on the captured thermal image
        font = cv2.FONT_HERSHEY_SIMPLEX
        cv2.putText(thermal_frame, f"Detected Temp: {detected_temperature}\u00b0C", (10, 30), font, 1, (0, 255, 0), 2)
        cv2.putText(thermal_frame, f"Disease: {matched_disease}", (10, 60), font, 1, (0, 255, 0), 2)
        ...
        # Show updated image with comparison in GUI
